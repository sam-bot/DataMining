...
public class Charm {
	...
	public static void main(String[] args) throws Exception {
		_inFile = args[0];
		Charm e = new Charm(_inFile, 0.001);
		System.out.println();
	}
	...
	// if one element in common, return true
	private Boolean checkDiffset(HashSet<Integer> t1, HashSet<Integer> t2) {
		for (int i : t2) {
			if (!t1.contains(i))
				return true;
		}
		...
		do {
			...
			for (int i = 0; i < tidSetCopy.size(); i++) {
				for (int k = i + 1; k < tidSetCopy.size(); k++) {
					...
					t1.clear();
					t1.addAll(hs);
					if ((hs.size() == size) && (!vertTemp.contains(t1))) {
						if (checkIntersect(verticalCopy.get(i),
								verticalCopy.get(k))
								&& (!verticalCopy.contains(intersect(
										verticalCopy.get(i),
										verticalCopy.get(k))))) {
							tidTemp.add(diffSet(tidSetCopy.get(k),
									tidSetCopy.get(i)));
							vertTemp.add(t1);
							itemCount++;
							moreSets = true;
	...
	private static ArrayList<Integer> intersect(ArrayList<Integer> f,
			ArrayList<Integer> s) {
		ArrayList<Integer> temp = new ArrayList<Integer>(f);
		temp.retainAll(s);
		return temp;
	}
